...
public class LambdaHandlerV2
{
	...
	public void purgeHandler(KinesisEvent event)
	{
	...
	public void parallelHandler(KinesisEvent event)
	{
		...
		try
		{
			Path dewePath = Paths.get(deweDir);
			Files.createDirectories(dewePath);
		} catch (Exception e)
		{
		...
		if (!binFiles.isEmpty())
		{
			try
			{
				...
				else
				{
					for (int i=0; i<binFiles.size(); i++)
					{
						download_one("bin", binFiles.get(i));
					}					
				}
			} catch (Exception e)
			{
		...
		if (!inFiles.isEmpty())
		{
			try
			{
				...
				else
				{
					for (int i=0; i<inFiles.size(); i++)
					{
						download_one("workdir", inFiles.get(i));
					}					
				}
			} catch (Exception e)
			{
		...
		try
		{
			if (!commands.isEmpty())
			{
				...
				{
					executor[i] = new Executor(deweDir + "/" + commands.get(i));
					executor[i].start();
				}
				for (int i=0; i<commands.size(); i++)
				{
	...
	public void download_one(String folder, String filename)
	{
			try
			{
				...
				String outfile = deweDir + "/" + filename;
					...
					while (!success)
					{
						...
						} catch (Exception e1)
						{
							...
							e1.printStackTrace();
							Thread.sleep(1000);
							runCommand("du -m", deweDir);
							runCommand("ls -l", deweDir);
						}
					}
				...
				{
					logger.debug(outfile + " is already there.");					
				}
			} catch (Exception e)
			{
				System.out.println(e.getMessage());
