...
public class LambdaHandlerV1
{
	...
	// Logging
	final static Logger logger = Logger.getLogger(LambdaHandlerV1.class);
...
//					{
						...
						} catch (Exception e1)
						{
							...
							e1.printStackTrace();
							sleep(1000);
							runCommand("du -m", binDir);
							runCommand("ls -l", binDir);
							runCommand("du -m", workDir);
							runCommand("ls -l", workDir);
						}
//					}
					
					if (folder.equals("bin"))
					{
						runCommand("chmod +x " + outfile, binDir);
					}					
				}
				else
				{
